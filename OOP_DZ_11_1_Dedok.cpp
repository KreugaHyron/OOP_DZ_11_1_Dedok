...
public:
    ...
    void spin(int& seed) {
        currentPosition = (currentPosition + 1) % SYMBOL_COUNT;
        buffer[currentPosition] = simpleRandom(seed) % SYMBOL_COUNT;
    }

    int getCurrentSymbol() const {
        return symbols[buffer[currentPosition]];
    }
};
